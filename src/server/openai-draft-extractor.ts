import type { DraftExtractor, MetricAlias, RecognitionDraft } from "@/server/recognition";

const nullableText = { type: ["string", "null"] };
const rowProperties = {
  metricDefinitionId: { type: ["integer", "null"] },
  originalName: { type: "string" }, displayName: { type: "string" },
  category: { type: "string" }, valueText: { type: "string" },
  unit: nullableText, referenceLow: nullableText, referenceHigh: nullableText,
  referenceText: nullableText, sourceText: { type: "string" },
  confidence: { type: "number", minimum: 0, maximum: 1 },
};
const properties = {
  detectedLanguage: { type: "string" }, laboratoryName: nullableText,
  collectedAt: nullableText, specimen: nullableText,
  warnings: { type: "array", items: { type: "string" } },
  observations: { type: "array", items: {
    type: "object", properties: rowProperties,
    required: Object.keys(rowProperties), additionalProperties: false,
  } },
};
...
export function createOpenAiDraftExtractor(
  ...
  return {
    async extract(text, aliases) {
      if (text.length > 200_000) throw new Error("Document exceeds AI text limit");
      const catalogue = [...new Map(aliases.map((alias) => [alias.metricDefinitionId, {
        id: alias.metricDefinitionId, name: alias.displayName,
        category: alias.category, unit: alias.defaultUnit,
      }])).values()];
...
All warnings, canonical names and categories must be Russian.`,
          ...
          text: { format: { type: "json_schema", name: "laboratory_draft", strict: true,
...
export function parseAiDraft(value: unknown, text: string, aliases: MetricAlias[], model: string): RecognitionDraft {
  ...
  for (const field of ["laboratoryName", "collectedAt", "specimen"] as const) {
    if (draft[field] !== null && typeof draft[field] !== "string") throw new Error("Invalid metadata");
  }
  if (typeof draft.detectedLanguage !== "string" || !Array.isArray(draft.warnings) ||
      draft.warnings.some((warning) => typeof warning !== "string") ||
      !Array.isArray(draft.observations) || draft.observations.length > 500) throw new Error("Invalid AI draft");
  const catalogue = new Map(aliases.map((alias) => [alias.metricDefinitionId, alias]));
  for (const row of draft.observations) {
    if (!row || typeof row !== "object") throw new Error("Invalid AI row");
    for (const field of ["originalName", "displayName", "category", "valueText", "sourceText"] as const) {
      if (typeof row[field] !== "string" || !row[field]!.trim() || row[field]!.length > 2000) throw new Error("Invalid AI row text");
    }
    for (const field of ["unit", "referenceLow", "referenceHigh", "referenceText"] as const) {
      if (row[field] !== null && typeof row[field] !== "string") throw new Error("Invalid AI row field");
    }
    if (!Number.isFinite(row.confidence) || row.confidence < 0 || row.confidence > 1) throw new Error("Invalid confidence");
    if (!text.includes(row.sourceText)) throw new Error("Missing source evidence");
  ...
  return { ...draft, extractedText: text, recognitionVersion: `openai-${model}-1`,
    warnings: [...draft.warnings, "AI подготовил черновик. Проверьте значения и сопоставления перед подтверждением."] };
}
